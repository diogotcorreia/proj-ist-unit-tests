...
import prrtests.utils.PoUILibTest;
...
public class ReadAfterImportTest {
    ...
    private abstract class GenericReadAfterImportTest extends PoUILibTest {
        ...
        protected void setupNetworkManager() {
            loadFromInputFile("test" + testId + ".input");
        }

        @Test
        void listClients() {
            this.interaction.addMenuOptions(3, 2);

            this.runApp();

            assertNoMoreExceptions();
            assertEquals(getTextFromFile("expected/test" + testId + "/clients.output"), this.interaction.getResult());
        }

        @Test
        void listTerminals() {
            this.interaction.addMenuOptions(4, 1);

            this.runApp();

            assertNoMoreExceptions();
            assertEquals(getTextFromFile("expected/test" + testId + "/terminals.output"), this.interaction.getResult());
        }

        // uncomment this and fix addMenuOptions arguments if you can already test it
        // @Test
        // void listCommunications() {
     ...
     */
    // @Nested
    // public class ReadFromEmptyNetworkTest extends PoUILibTest {
    //     protected void setupNetworkManager() {
    //     }

    //     @ParameterizedTest(name = "empty Network: {0}")
...
//                 "013,STEVE_INV,Show batches by partner from from a Network with more batches"
//         })
...
//                 "013,STONE_PICKAXE,Show batch by product from a Network with more batches"
//         })
